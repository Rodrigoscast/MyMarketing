 ...
 */
async function provisionOrganization(userId, company, fullName) {
    ...
    const { data: existing } = await supabase
        ...
        .limit(1)
        .maybeSingle();
    const existingOrg = existing ? pickOrg(existing.organizations) : null;
    if (!existingOrg) {
        // Cria a organização
...
async function getFirstOrganization(userId) {
    ...
    if (error || !data?.organizations)
        return null;
    return pickOrg(data.organizations);
}
...
function sessionToData(session, user, organization) {
    return {
