...
export async function publishVideoToYouTube(params) {
    ...
    const { youtube, channelId } = await getAuthenticatedYouTubeClient(params.organizationId, params.socialAccountId);
    ...
    try {
        ...
        throw new AppError(`Falha no upload para YouTube: ${err instanceof Error ? err.message : "Erro desconhecido"}`, 500, "YOUTUBE_UPLOAD_FAILED");
    }
}
/**
...
export async function updateYouTubeVideoMetadata(params) {
    const { youtube } = await getAuthenticatedYouTubeClient(params.organizationId, params.socialAccountId);
    ...
    if (!existing)
        throw new NotFoundError("Vídeo no YouTube");
    const updated = {
        id: params.youtubeVideoId,
        snippet: {
        ...
        recordingDetails: params.metadata.recordingDate || params.metadata.locationLat || params.metadata.locationLng
            ? {
