...
router.post("/:id/clone", async (req, res) => {
    ...
    `)
        ...
        .eq("organization_id", req.organizationId)
        .single();
    if (error || !originalVideo)
        throw new NotFoundError("Vídeo original");
    ...
    if (payload.socialAccountId) {
        const { data: channel } = await supabase
            ...
            .single();
        if (!channel)
            throw new NotFoundError("Canal do YouTube de destino");
    ...
    if (createError) {
        ...
        if (payload.copyAsset && newAssetId !== originalVideo.asset_id) {
            const { data: newAsset } = await supabase
                ...
                .single();
            if (newAsset)
                await fs.unlink(newAsset.storage_path).catch(() => { });
...
router.post("/bulk", async (req, res) => {
    ...
    if (!videos || videos.length === 0)
        throw new NotFoundError("Nenhum vídeo encontrado");
