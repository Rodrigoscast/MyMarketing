...
/**
 * Worker de Analytics - coleta métricas e comentários do YouTube
 * Deve ser executado periodicamente (ex: a cada 6 horas via cron)
 */
...
async function fetchVideoMetrics(
  ...
  try {
    ...
    trafficSources = Object.fromEntries(
      (trafficResponse.data.rows ?? []).map((r: any[]) => [r[0], Number(r[1])])
    );
  } catch {
  ...
  // Geografia
  let geography: Record<string, number> = {};
  try {
    ...
    geography = Object.fromEntries(
      (geoResponse.data.rows ?? []).map((r: any[]) => [r[0], Number(r[1])])
    );
  } catch {
  ...
  let deviceTypes: Record<string, number> = {};
  try {
    ...
    deviceTypes = Object.fromEntries(
      (deviceResponse.data.rows ?? []).map((r: any[]) => [r[0], Number(r[1])])
    );
  } catch {
...
async function fetchVideoComments(
  ...
  const comments = (response.data.items ?? [])
    .filter((item): item is youtube_v3.Schema$CommentThread & { snippet: youtube_v3.Schema$CommentThreadSnippet & { topLevelComment: youtube_v3.Schema$Comment } } =>
    ...
    .map((item) => {
      ...
      const replies = item.replies?.comments
        ?.filter((reply): reply is youtube_v3.Schema$Comment => !!reply.snippet && !!reply.id) ?? [];
 ...
 */
export async function collectAnalyticsForOrg(organizationId: string): Promise<{
  ...
  for (const video of videos) {
    ...
    try {
      ...
      const { youtube, youtubeAnalytics } = await getAuthenticatedYouTubeClient(
        organizationId,
        video.social_account_id
      );
 ...
 */
export async function collectAnalyticsForVideo(
  organizationId: string,
  socialAccountId: string,
  videoDbId: string
): Promise<{ metricsCollected: boolean; commentsCollected: number; errors: string[] }> {
  const supabase = getSupabase();
  const errors: string[] = [];
 ...
 */
export async function runAnalyticsCollection(): Promise<void> {
  ...
  for (const orgId of orgIds) {
    try {
      ...
      console.log(
        `[Analytics] Org ${orgId}: ${result.videosProcessed} vídeos, ${result.metricsCollected} métricas, ${result.commentsCollected} comentários`
      );
